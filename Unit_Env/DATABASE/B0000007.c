...
#elif defined(VCAST_USE_SETLINEBUF)
...
/* Add a prototype for the user's function.  We define this
   to match the prototype for the stdio.h function "puts"
*/
#if defined (VCAST_CUSTOM_OUTPUT)
int VCAST_CUSTOM_OUTPUT (const char* S);  
#endif
...
void vectorcast_write_to_std_out (const char *s) {
   ...
   if ((vcast_output_buffer_end + VCAST_strlen (s)) >= VCAST_OUTPUT_BUFFER_SIZE) {
      ...
      if (vcast_check_for_dump_buffer ()==0) {
         VCAST_driver_termination( 7, 1014 );
         }
      }
...
#if VCAST_HAS_FLOAT128 && VCAST_HAS_QUADMATH && !defined(VCAST_FP) && !defined(VCAST_NO_SPRINTF)
#include <quadmath.h>
#endif /* endif VCAST_HAS_FLOAT128 && VCAST_HAS_QUADMATH && !defined(VCAST_FP) && !defined(VCAST_NO_SPRINTF)*/
...
#else
#if VCAST_HAS_FLOAT128 && VCAST_HAS_QUADMATH
...
#endif /* endif VCAST_HAS_FLOAT128 && VCAST_HAS_QUADMATH */
#endif      /* endif VCAST_NO_FLOAT */
} 
