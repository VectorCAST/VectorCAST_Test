...
#ifndef VCAST_DISABLE_STD_STRING_DETECTION
#ifndef VCAST_DISABLE_TI_STRING
#ifdef __cplusplus

/* Support for the ANSI C++ Standard string */
#include <string>

void VCAST_TI_STD_STRING ( vcast_std_string vcast_param ) {
   std::string *vcast_str = static_cast<std::string*>(vcast_param);
   switch (vCAST_COMMAND) {
      case vCAST_PRINT:
         if (vCAST_COMMAND_IS_MIN_MAX == vCAST_false){ 
            char *vcast_char = const_cast<char*>(vcast_str->c_str ());
            int vcast_length = vcast_str->length ();
            VCAST_TI_STRING ( &vcast_char, vcast_length, 0, -1 );
         } /* end if */

         break;
      case vCAST_SET_VAL:
         *vcast_str = VCAST_convert ( &vCAST_PARAMETER[vCAST_INDEX+1] );
         break; 
      case vCAST_FIRST_VAL:
         if ( vCAST_COMMAND_IS_MIN_MAX ) {
            vectorcast_fprint_integer(vCAST_OUTPUT_FILE,0);
      ...
      case vCAST_LAST_VAL :
         if ( vCAST_COMMAND_IS_MIN_MAX ) {
            int vcast_length = vcast_str->length ();
            vectorcast_fprint_integer(vCAST_OUTPUT_FILE,vcast_length-1);
            vectorcast_fprint_string_with_cr(vCAST_OUTPUT_FILE,"");
         }
         break;
      default :
...
#endif /* __cplusplus */
#endif /* VCAST_DISABLE_TI_STRING */
#endif /* VCAST_DISABLE_STD_STRING_DETECTION */

#ifndef VCAST_DISABLE_STD_WSTRING_DETECTION
#ifndef VCAST_DISABLE_TI_STRING
#ifdef __cplusplus

/* Support for the ANSI C++ Standard wstring */
#include <string>

void VCAST_TI_STD_WSTRING ( vcast_std_wstring vcast_param ) {
   std::wstring *vcast_wstr = static_cast<std::wstring*>(vcast_param);
   switch (vCAST_COMMAND) {
      case vCAST_PRINT:
         if (vCAST_COMMAND_IS_MIN_MAX == vCAST_false){ 
            /* Convert the wstring to a string. This is only useful
             * for ansi encoding. If another type of encoding is 
             * necessary, use user code. */
            std::string vcast_str;
            std::wstring::iterator vcast_iter = vcast_wstr->begin ();
            for (; vcast_iter != vcast_wstr->end (); ++vcast_iter) {
               vcast_str += (char)*vcast_iter;
            }
            char *vcast_char = const_cast<char*>(vcast_str.c_str ());
            int vcast_length = vcast_str.length ();
            VCAST_TI_STRING ( &vcast_char, vcast_length, 0, -1 );
      ...
      case vCAST_SET_VAL:
      {
         char *vcast_ret = VCAST_convert ( &vCAST_PARAMETER[vCAST_INDEX+1] );
         while (vcast_ret && *vcast_ret) {
            vcast_wstr->append (1, *vcast_ret);
            ++vcast_ret;
         }
         break;  
      }
      ...
      case vCAST_LAST_VAL :
         if ( vCAST_COMMAND_IS_MIN_MAX ) {
            int vcast_length = vcast_wstr->length ();
            vectorcast_fprint_integer(vCAST_OUTPUT_FILE,vcast_length-1);
            vectorcast_fprint_string_with_cr(vCAST_OUTPUT_FILE,"");
         }
         break;
...
#endif /* __cplusplus */
#endif /*VCAST_DISABLE_TI_STRING*/
#endif /* VCAST_DISABLE_STD_WSTRING_DETECTION */
